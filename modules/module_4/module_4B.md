...
|T6|Image Processing|In-program data setup. Import and align photos including spare point cloud generation and clean-up. Dense point cloud generation (may be skipped if a polygon mesh is to be calculated. However, meshes from dense clouds are usually superior). Polygon mesh generation (optional). Texture generation (optional). Data export: you can export the completed model as an OBJ file, which can then also be uploaded into Sketchfab. You can also export the model as a PDF file and other formats.|
